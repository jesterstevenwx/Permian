...
for seq, syr, smon, sday, shr, eyr, emon, eday, ehr in zip(seqs, syrs, smons, sdays, shrs, eyrs, emons, edays, ehrs):
    with open(f'namelist.input.{f"{seq:02}" if isinstance(seq, int) else seq}','a') as fl:
        ...
        if ndown and seq == 'ndown':
            fl.write(f'io_form_auxinput2                   = 2\n')
        ...
        if ndown and isinstance(seq, int):
            fl.write(f'max_dom                             = 1,\n')
            fl.write(f's_we                                = 1,     1,     1,\n')
            fl.write(f'e_we                                = {e_we_d02},   161,   161,\n')
            fl.write(f's_sn                                = 1,     1,     1,\n')
            fl.write(f'e_sn                                = {e_sn_d02},   161,   161,\n')
            fl.write(f's_vert                              = 1,     1,     1,\n')
            fl.write(f'e_vert                              = 80,   80,     80,\n')
        else:
            ...
            fl.write(f'e_vert                              = 80,   80,   80,     80,\n')
        fl.write(f'sfcp_to_sfcp                         = .false.\n')
        if ndown and isinstance(seq, int):
            fl.write(f'dx                                  = 800,   160,   32,\n')
            fl.write(f'dy                                  = 800,   160,   32,\n')
        else:
        ...
        fl.write(f'auto_levels_opt                     = 2,\n')
        fl.write(f'max_dz                              = 1000,\n')
        fl.write(f'dzbot                               = 30,\n')
        fl.write(f'dzstretch_s                         = 1.11,\n')
        fl.write(f'dxstretch_u                         = 1.06,\n')
        fl.write(f'smooth_option                       = 0\n')
